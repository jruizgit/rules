exports = module.exports = durable = function () {
    ...
    var ruleSet = function(name, host, rules) {
        ...
        var postMessages = function(messages, names, index, s, complete) {
            if (index == names.length) {
                ...
                var ruleset = names[index];
                var messageList = messages[ruleset];
                if (messageList.length == 1) {
                    host.post(ruleset, messageList[0], function(err) {
                        if (err) {
                            complete(err, s);
                        } else {
                    ...
                    host.postBatch(ruleset, messageList, function(err) {
                        if (err) {
                            complete(err, s);
                        } else {
                            postMessages(messages, names, ++index, s, complete);
    ...
    var host = function () {
        ...
        that.start = function(ruleset, state, complete) {
            var rules = rulesDirectory[ruleset];
            if (rules) {

exports = module.exports = durable = function () {
    ...
    var ruleset = function(name, host, rulesetDefinition) {
        ...
        for (var actionName in rulesetDefinition) {
            ...
            if (rule.resolve) {
                if (!host.actionResolver) {
                    throw 'no action resolver provided';
                }

                actions[actionName] = promise(host.actionResolver(rule.resolve));
            } else {
    ...
    function run(rulesetDefinitions, basePath, databases, start) {
        var rulesHost = host();

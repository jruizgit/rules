exports = module.exports = durable = function () {
    ...
    var ruleSet = function (namespace, rules) {
        ...
        var alpha = function (t) {
            ...
            that.addssert = function (transaction, sid, resourceKey, acHash, parentName, c, actions) {
                if (that.test(c)) {
                    ...
                    for (var i = 0; i < nextAssertions.length; ++i) {
                        if (handled) {
                            nextAssertions[i](transaction, sid, parentName, c, actions);
                        } else if (nextAddssertions[i](transaction, sid, resourceKey, acHash, parentName, c, actions)) {
                            handled = true;
                        }
                    }
        ...
        that.getStatus = function (partition, sid, complete) {
            partition.db.hget(sessionDirectoryKey, sid, complete);
        }
    ...
    var host = function () {
        ...
        var getStatus = function (programName, sid, complete) {
            var program = programDirectory[programName];
            if (program) {
                resolvePartition(programName, sid , function (err, result) {
                    if (err) {
                        complete(err);
                    } else {
                        program.getStatus(result, sid, complete);
                    }
                });
            } else {

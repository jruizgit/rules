exports = module.exports = durable = function () {
    ...
    var ruleSet = function(name, host, rules) {
        ...
        var postMessages = function(messages, index, s, complete) {
            if (index == messages.length) {
                complete(null, s);
            } else {
                host.post(messages[index], function(err) {
                    if (err) {
                        complete(err, s);
                    }

                    postMessages(messages, ++index, s);
                });
            }
        }
            ...
            if (!result) {
                ...
                action.run(s, function(err, s) {
                    if (err) {
                        ...
                        postMessages(messages, 0, s, function(err, s) {
                            if (err) {
                                try {
                                    r.abandonAction(handle, s.getHandle());
                                } catch (reason) {
    ...
    var host = function () {
        ...
        that.post = function (message, complete) {
            var rules = rulesDirectory[message.ruleset];
            if (rules) {
                rules.assertEvent(message, complete);
            } else {
                complete('Could not find ' + message.ruleset);
            }
        };
        ...
        that.run = function (basePath, databases, start) {
            ...
            app.post(basePath + "/:rules/:sid", function (request, response) {
                ...
                message.rules = request.params.rules;

exports = module.exports = durable = function () {
    ...
    var ruleset = function(name, host, rules) {
        ...
        var startBranches = function(branches, names, index, s, complete) {
            if (index == names.length) {
                ...
                host.start(branchName, state, function(err) {
                    if (err) {
                        complete(err, s);
                    } else {
        ...
        for (var actionName in rules) {
            ...
            if (typeof(rule.run) === 'function') {
                ...
                actions[actionName] = fork(parseDefinitions(rule.run, host, name));
            }
                    ...
                    if (typeof(stage.to) === 'string') {
                        ...
                        if (!nextStage.run) {
                            ...
                            if (typeof(nextStage.run) === 'function' || nextStage.run.continueWith) {
                                ...
                                rule.run = to(stage.to).continueWith(fork(parseDefinitions(nextStage.run, host, namespace), function (pName) {
                                    return (pName !== 'label');
                                }));
                            }
                        }
                        ...
                        for (var transitionName  in stage.to) {
                            ...
                            if (!nextStage.run) {
                                ...
                                if (typeof(nextStage.run) === 'function' || nextStage.run.continueWith) {
                                    ...
                                    rule.run = to(transitionName).continueWith(fork(parseDefinitions(nextStage.run, host, namespace), function (pName) {
                                        return (pName !== 'label');
                                    }));
                                }
                            }
    ...
    var host = function () {
        ...
        var getStatus = function (rulesetName, sid, complete) {
            var rules = rulesDirectory[rulesetName];
            if (rules) {
            } else {
                complete('Could not find ' + rulesetName);
            }
        };

        var patchSession = function (rulesetName, document, complete) {
        };

        var getRuleset = function (rulesetName, complete) {
            var rules = rulesDirectory[rulesetName];
            if (rules) {
                complete(null, rules);
            } else {
        ...
        that.register = function (rulesetName, rules) {
            if (rulesDirectory[rulesetName]) {
                throw 'rules ' + rulesetName + ' already registered';
            } else {
                rulesDirectory[rulesetName] = rules;
        ...
        that.postBatch = function (rules, messages, complete) {
            var rules = rulesDirectory[rules];
            if (rules) {
                rules.assertEvents(messages, complete);
            } else {
                complete('Could not find ' + rules);
            }   
        }

        that.post = function (rules, message, complete) {
            var rules = rulesDirectory[rules];
            if (rules) {
        ...
        that.start = function(rules, state, complete) {
            var rules = rulesDirectory[rules];
            if (rules) {

...
"        else\n"
"            if message_cache[new_mid] then\n"
"                frame[name] = message_cache[new_mid]\n"
"            else\n"
...
"                end\n"
"                if packed_message then\n"
"                    local unpacked_message = cmsgpack.unpack(packed_message)\n"
"                    frame[name] = unpacked_message\n"
"                    message_cache[new_mid] = unpacked_message\n"
"                else\n"
"                    return nil\n"
"                end\n"
"            end\n"
...
"        frame[\"$f\"] = 1\n"
"    end\n"
"    redis.call(\"%s\", results_key, cmsgpack.pack(frame))\n"
"    for name, message in pairs(frame) do\n"
...
"        local frames_d_key = keys[index] .. \"!d!\" .. sid\n"
"        local packed_frame_list_len = redis.call(\"llen\", frames_key)\n"
"        redis.call(\"del\", frames_d_key)\n"
...
"return true\n",
                         ...
                         name,
                         name,
                         currentNode->value.c.span > 0 ? "rpush" : "lpush",
                         lua)  == -1) {
                return ERR_OUT_OF_MEMORY;
            }
            free(oldLua);
...
"            elseif not message_cache[message[\"id\"]] and not redis.call(\"hget\", facts_hashset, message[\"id\"]) then\n"
"                result = false\n"
"            else\n"

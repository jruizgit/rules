...
Durable.ruleset :match1 do
  when_all (m.subject.matches("a+(bc|de?)*")) do
    puts "match1 -> #{m.subject}"
  end
  when_start do
...
Durable.ruleset :match2 do
  ...
  when_start do
    post :match2, {:id => 1, :sid => 1, :ip => "73.60.124.136"}
    post :match2, {:id => 2, :sid => 1, :ip => "256.60.124.136"}
    post :match2, {:id => 3, :sid => 1, :ip => "250.60.124.256"}
    post :match2, {:id => 4, :sid => 1, :ip => "73.60.124"}
    post :match2, {:id => 5, :sid => 1, :ip => "127.0.0.1"}
  end
end
...
Durable.ruleset :match3 do
  ...
  when_start do
    post :match3, {:id => 1, :sid => 1, :url => "https://github.com"}
    post :match3, {:id => 2, :sid => 1, :url => "http://github.com/jruizgit/rul!es"}
    post :match3, {:id => 3, :sid => 1, :url => "https://github.com/jruizgit/rules/blob/master/docs/rb/reference.md"}
    post :match3, {:id => 4, :sid => 1, :url => "//rules"}
    post :match3, {:id => 5, :sid => 1, :url => "https://github.c/jruizgit/rules"}
  end
end

Durable.ruleset :match4 do
  when_all (m.subject.matches("%u+")) do
    puts "match 4 upper -> #{m.subject}"
  end
  when_all (m.subject.matches("%l+")) do
    puts "match 4 lower -> #{m.subject}"
  end
  when_start do
    post :match4, {:id => 1, :sid => 1, :subject => "ABCDZ"}
    post :match4, {:id => 2, :sid => 1, :subject => "W\u00D2\u00D3"}
    post :match4, {:id => 3, :sid => 1, :subject => "abcz"}
    post :match4, {:id => 4, :sid => 1, :subject => "wßÿ"}
    post :match4, {:id => 5, :sid => 1, :subject => "AbcZ"}
  end
end

...
Durable.ruleset :a1 do
  when_ (m.amount < 1000) | (m.amount > 10000) do
...
Durable.statechart :a2 do
  state :input do
    to :denied, when_((m.subject == 'approve') & (m.amount > 1000)) do
      puts "a2 state denied: #{s.id}"
    end
    to :pending, when_((m.subject == 'approve') & (m.amount <= 1000)) do
  ...
  state :pending do
    ...
    to :approved, when_(s.status == 'approved')
...
Durable.flowchart :a3 do
  stage :input
  to :request, when_((m.subject == 'approve') & (m.amount <= 1000))
  to :deny, when_((m.subject == 'approve') & (m.amount > 1000))
  
  stage :request do
    puts "a3 flow requesting approval for: #{s.id}"
    s.status? ? s.status = "approved": s.status = "pending"
  end
  to :approve, when_(s.status == 'approved')
...
Durable.statechart :a6 do
  ...
  state :work do   
    state :enter do
      to :process, when_(m.subject == "enter") do
        puts "a6 continue process"
      end
    end
    state :process do
      to :process, when_(m.subject == "continue") do
        puts "a6 processing"
      end
    end
    to :work, when_(m.subject == "reset") do
      puts "a6 resetting"
    end
    to :canceled, when_(m.subject == "cancel") do
      puts "a6 canceling"
    end
  end
  state :canceled
